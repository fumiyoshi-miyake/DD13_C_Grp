...
class Sensor(object):
  ...
  def __init__(self, spi_dev = "/dev/spidev0.0"):
    ...
    self.__txbuf = np.zeros(Sensor.VOSPI_FRAME_SIZE, dtype=np.uint16)
    ...
    self.__xmit_buf = np.zeros((self.__msg_size * Sensor.ROWS), dtype=np.uint8)
    ...
    for i in range(Sensor.ROWS):
      self.__xmit_struct.pack_into(self.__xmit_buf, i * self.__msg_size,
        ...
        self.__capture_buf.ctypes.data + Sensor.VOSPI_FRAME_SIZE_BYTES * i,  #   __u64     rx_buf;
        Sensor.VOSPI_FRAME_SIZE_BYTES,                                      #   __u32     len;
        Sensor.SPEED,                                                       #   __u32     speed_hz;
        0,                                                                  #   __u16     delay_usecs;
        Sensor.BITS,                                                        #   __u8      bits_per_word;
        1,                                                                  #   __u8      cs_change;
        0)                                                                  #   __u32     pad;
    ...
    ioctl(self.__handle, SPI_IOC_RD_MODE, struct.pack("=B", Sensor.MODE))
    ioctl(self.__handle, SPI_IOC_WR_MODE, struct.pack("=B", Sensor.MODE))

    ioctl(self.__handle, SPI_IOC_RD_BITS_PER_WORD, struct.pack("=B", Sensor.BITS))
    ioctl(self.__handle, SPI_IOC_WR_BITS_PER_WORD, struct.pack("=B", Sensor.BITS))

    ioctl(self.__handle, SPI_IOC_RD_MAX_SPEED_HZ, struct.pack("=I", Sensor.SPEED))
    ioctl(self.__handle, SPI_IOC_WR_MAX_SPEED_HZ, struct.pack("=I", Sensor.SPEED))
  ...
  def capture_segment(handle, xs_buf, xs_size, capture_buf):
    ...
    while messages > 0:
      if messages > Sensor.SPIDEV_MESSAGE_LIMIT:
        count = Sensor.SPIDEV_MESSAGE_LIMIT
      else:
        count = messages
  ...
  def update(self, log_time = False, debug_print = False, retry_reset = True):
    ...
    while True:
      while True:
        Sensor.capture_segment(self.__handle, self.__xmit_buf, self.__msg_size, self.__capture_buf[0])
